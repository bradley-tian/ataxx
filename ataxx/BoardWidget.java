...
class BoardWidget extends Pad {
    ...
    /**
     * Draws the initial setup of the board.
     */
    @Override
    public synchronized void paintComponent(Graphics2D g) {
        g.setColor(BLANK_COLOR);
        g.fillRect(0, 0, _dim, _dim);
        for (int i = SQDIM; i < SQDIM * SIDE; i += SQDIM) {
            g.setColor(LINE_COLOR);
            g.setStroke(LINE_STROKE);
            g.drawLine(i, 0, i, SQDIM * SIDE);
            g.drawLine(0, i, SQDIM * SIDE, i);
        }
        for (char row = '1'; row <= '7'; row += 1) {
            for (char col = 'a'; col <= 'g'; col += 1) {
                if (_model.get(Board.index(col, row)) == RED) {
                    g.setColor(RED_COLOR);
                    g.fillOval(SQDIM * (col - 'a') + (SQDIM - PIECE_RADIUS * 2) / 2,
                            SQDIM * (row - '1') + + (SQDIM - PIECE_RADIUS * 2) / 2,
                            PIECE_RADIUS * 2, PIECE_RADIUS * 2);
                }
                if (_model.get(Board.index(col, row)) == BLUE) {
                    g.setColor(BLUE_COLOR);
                    g.fillOval(SQDIM * (col - 'a') + (SQDIM - PIECE_RADIUS * 2) / 2,
                            SQDIM * (row - '1') + (SQDIM - PIECE_RADIUS * 2) / 2,
                            PIECE_RADIUS * 2, PIECE_RADIUS * 2);
                }
                if (_model.get(Board.index(col, row)) == BLOCKED) {
                    drawBlock(g, SQDIM * (col - 'a'), SQDIM * (row - '1'));
                }
            }
        }
    }
    ...
    /**
     * Draw a block centered at (CX, CY) on G. I need to fix this.
     *
     * @param g  Graphics
    ...
    void drawBlock(Graphics2D g, int cx, int cy) {
        ...
        g.drawLine(cx + (SQDIM - BLOCK_WIDTH) / 2,
                cy + (SQDIM - BLOCK_WIDTH) / 2,
                cx + (SQDIM - BLOCK_WIDTH) / 2 + BLOCK_WIDTH,
                cy + (SQDIM - BLOCK_WIDTH) / 2);
        g.drawLine(cx + (SQDIM - BLOCK_WIDTH) / 2,
                cy + (SQDIM - BLOCK_WIDTH) / 2,
                cx + (SQDIM - BLOCK_WIDTH) / 2,
                cy + (SQDIM - BLOCK_WIDTH) / 2 + BLOCK_WIDTH);
        g.drawLine(cx + (SQDIM - BLOCK_WIDTH) / 2 + BLOCK_WIDTH,
                cy + (SQDIM - BLOCK_WIDTH) / 2,
                cx + (SQDIM - BLOCK_WIDTH) / 2 + BLOCK_WIDTH,
                cy + (SQDIM - BLOCK_WIDTH) / 2 + BLOCK_WIDTH);
        g.drawLine(cx + (SQDIM - BLOCK_WIDTH) / 2,
                ...
                cx + (SQDIM - BLOCK_WIDTH) / 2 + BLOCK_WIDTH,
                cy + (SQDIM - BLOCK_WIDTH) / 2 + BLOCK_WIDTH);
        g.drawLine(cx + (SQDIM - BLOCK_WIDTH) / 2 + BLOCK_WIDTH / 2,
                cy + (SQDIM - BLOCK_WIDTH) / 2,
                cx + (SQDIM - BLOCK_WIDTH) / 2 + BLOCK_WIDTH / 2,
                cy + (SQDIM - BLOCK_WIDTH) / 2 + BLOCK_WIDTH);
        g.drawLine(cx + (SQDIM - BLOCK_WIDTH),
                cy + (SQDIM - BLOCK_WIDTH) / 2 + BLOCK_WIDTH / 2,
                cx + (SQDIM - BLOCK_WIDTH) / 2 + BLOCK_WIDTH,
                cy + (SQDIM - BLOCK_WIDTH) / 2 + BLOCK_WIDTH / 2);
        g.drawLine(cx + (SQDIM - BLOCK_WIDTH) / 2,
                cy + (SQDIM - BLOCK_WIDTH) / 2,
     ...
     * Fix after command and selectedCol.
     */
        ...
        if (where.getButton() == MouseEvent.BUTTON1) {
            ...
            if (mouseCol >= 'a' && mouseCol <= 'g'
                ...
                if (_blockMode) {
                    ...
                    if (_selectedCol != 0) {
                        String cmd = Character.toString(_selectedCol)
                                + Character.toString(_selectedRow)
                                + "-"
                                + Character.toString(mouseCol)
                                + Character.toString(mouseRow);
